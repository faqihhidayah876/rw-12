...
import { Loader2, Search, Image as ImageIcon, Edit, Trash2, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
const DataWarga = () => {
  ...
  // State baru khusus untuk menampung kordinat saat diedit
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      // PERHATIKAN: Kita menambahkan penarikan id, koordinat_lat, koordinat_lng dari tabel rumah
  ...
  const handleDelete = async (id, nama) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus data warga bernama ${nama}?`)) return;
    try {
  ...
  // FUNGSI UPDATE BARU (Memperbarui Warga & Lokasi Rumah)
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      alert('Data dan lokasi berhasil diperbarui!');
      setEditData(null);
  ...
  const openEditModal = (warga) => {
    setEditData(warga);
    setEditLocation({
      lat: warga.keluarga?.rumah?.koordinat_lat || 0.5333, // Default Pekanbaru jika kosong
                    ...
                    {isAdmin && (
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          {/* Memanggil openEditModal untuk menyiapkan peta */}
                          <button onClick={() => openEditModal(warga)} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors rounded-md hover:bg-blue-50" title="Edit"><Edit size={16} /></button>
                          <button onClick={() => handleDelete(warga.id, warga.nama_lengkap)} className="p-1.5 text-slate-400 hover:text-red-600 transition-colors rounded-md hover:bg-red-50" title="Hapus"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    )}
                  </tr>
        ...
        {!isLoading && filteredData.length > 0 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 bg-slate-50">
            ...
            <div className="flex gap-2">
              <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-1.5 rounded-md border border-slate-300 text-slate-600 disabled:opacity-50 disabled:bg-slate-100 hover:bg-white"><ChevronLeft size={18}/></button>
              <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="p-1.5 rounded-md border border-slate-300 text-slate-600 disabled:opacity-50 disabled:bg-slate-100 hover:bg-white"><ChevronRight size={18}/></button>
            </div>
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editData.nama_lengkap} onChange={e => setEditData({...editData, nama_lengkap: e.target.value})} required className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">NIK</label>
                  <input type="number" value={editData.nik} onChange={e => setEditData({...editData, nik: e.target.value})} required className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={editData.status_warga} onChange={e => setEditData({...editData, status_warga: e.target.value})} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none">
              ...
              <div className="mt-6">
                <label className="block text-xs font-bold text-slate-700 mb-2">Geser & Klik Peta untuk Mengubah Lokasi Rumah</label>
                ...
                <p className="text-[10px] text-slate-500 mt-1.5 font-medium">Titik biru menandakan lokasi yang akan disimpan.</p>
              </div>
