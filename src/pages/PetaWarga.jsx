...
const PetaWarga = () => {
  ...
  const [mapInstance, setMapInstance] = useState(null); // Menyimpan instance peta
  
  // State untuk Fitur Pencarian
  const [searchTerm, setSearchTerm] = useState('');
  const [isSearchOpen, setIsSearchOpen] = useState(false); // Untuk tampilan mobile
  ...
  // --- 1. FITUR MICRO-MARKER (Menggunakan Tailwind didalam Peta!) ---
  const createCustomIcon = (isBerpenghuni, blok) => {
    return L.divIcon({
      className: 'bg-transparent border-none', // Matikan style bawaan Leaflet
      // Buat lingkaran kecil rapi dengan teks blok di tengahnya
  ...
  const flyToHouse = (rumah) => {
    if (mapInstance) {
      // Terbang ke lokasi dengan zoom level 21 (sangat dekat)
      mapInstance.flyTo([rumah.koordinat_lat, rumah.koordinat_lng], 21, {
        animate: true,
        duration: 1.5 // Durasi animasi terbang (detik)
      });
      
      // Jika rumah ada riwayat keluarganya, langsung buka pop-up detailnya!
      if (rumah.adaKeluarga) {
        setTimeout(() => {
          setSelectedDetail(rumah);
        }, 800); // Buka setelah animasi terbang hampir selesai
      }
    }
    // Tutup panel pencarian jika di HP
  ...
  return (
    <div className="relative w-full h-screen bg-slate-100 overflow-hidden">
      ...
      {/* --- PETA INTERAKTIF --- */}
      {isLoading ? (
        <div className="flex items-center justify-center w-full h-full"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div></div>
      ) : (
        <MapContainer 
          center={mapCenter} 
          zoom={17} 
          ref={setMapInstance} // Mengaitkan map ke state untuk dikendalikan (flyTo)
          style={{ height: '100%', width: '100%', zIndex: 10 }}
        >
          {/* FITUR OVERZOOM: maxNativeZoom memanjangkan gambar peta agar bisa dizoom tanpa abu-abu */}
          <TileLayer 
          ...
          {dataRumah.map((rumah) => {
            ...
            return (
              <Marker 
                ...
                icon={createCustomIcon(rumah.isBerpenghuni, rumah.blok_nomor)} // Pakai icon mungil yang kita buat
              >
                {/* Pop-up Mini (Opsional jika di-klik manual) */}
                <Popup className="custom-popup border-none rounded-xl">
                  <div className="p-1 min-w-[180px]">
                    <div className="flex items-center gap-2 border-b border-slate-100 pb-2 mb-2">
                      ...
                      {keluargaUtama && <span className={`ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full ${rumah.isBerpenghuni ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'}`}>RT {keluargaUtama.rt}</span>}
                    </div>
                    ...
                    {rumah.isBerpenghuni ? (
                      ...
                      <div className="mb-3 text-center py-2 bg-red-50 rounded-lg"><AlertCircle size={20} className="text-red-500 mx-auto mb-1" /><p className="text-xs font-bold text-red-700">Kosong / Historis</p></div>
                    )}

                    <button onClick={() => setSelectedDetail(rumah)} disabled={!rumah.adaKeluarga} className={`w-full py-2 text-white text-xs font-bold rounded-lg transition-colors ${!rumah.adaKeluarga ? 'bg-slate-300 cursor-not-allowed' : (rumah.isBerpenghuni ? 'bg-blue-600 hover:bg-blue-700' : 'bg-red-600 hover:bg-red-700')}`}>
      ...
      {selectedDetail && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh] border border-slate-100">
            
            <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50">
              <div>
                <h3 className="font-extrabold text-slate-900 text-lg">Detail Penghuni Rumah</h3>
                <p className="text-sm text-slate-500 font-medium flex items-center gap-1"><MapPin size={14}/> Blok {selectedDetail.blok_nomor}</p>
              </div>
              <button onClick={() => setSelectedDetail(null)} className="p-1.5 text-slate-400 hover:text-red-600 bg-white border border-slate-200 rounded-lg hover:border-red-200 transition-colors">
                <X size={20} />
              </button>
            </div>
              ...
              {selectedDetail.keluarga?.map((kel) => (
                <div key={kel.id} className="mb-6 last:mb-0">
                  <div className="flex items-center gap-2 mb-3 bg-slate-100 p-2.5 rounded-xl border border-slate-200">
                    <div className="bg-slate-500 p-1.5 rounded-lg"><User size={16} className="text-white"/></div>
                    <div>
                      <p className="text-xs text-slate-600 font-bold uppercase tracking-wider">Keluarga Terdaftar (RT {kel.rt})</p>
                  ...
                  <div className="space-y-3 pl-2 border-l-2 border-slate-100 ml-4">
                    {kel.warga?.map((w) => (
                      <div key={w.id} className={`flex justify-between items-center p-3 rounded-xl border shadow-sm relative ${w.status_warga === 'Hidup' ? 'bg-white border-slate-200' : 'bg-red-50 border-red-100'}`}>
                        ...
                        <div>
                          <p className={`font-bold text-sm ${w.status_warga === 'Hidup' ? 'text-slate-800' : 'text-red-700'}`}>
                            {w.nama_lengkap} 
                            {w.status_warga !== 'Hidup' && <span className="ml-2 text-[10px] bg-red-200 text-red-700 px-1.5 py-0.5 rounded font-extrabold uppercase">[{w.status_warga}]</span>}
                          </p>
                          <p className="text-xs text-slate-500 font-medium mt-0.5">{w.nik}</p>
                        </div>
                        <div className="text-right">
                          <span className={`inline-block px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase tracking-wide ${w.hubungan_keluarga === 'Kepala Keluarga' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600'}`}>
                            {w.hubungan_keluarga}
                          </span>
                        </div>
                      </div>
